...
public:
    ...
    uint8_t getNumVals() { return possible_values.size(); }
...
public:
    ...
    void printBoard() {
        ...
        for (uint8_t i = 0; i < 3; i++) {
            std::cout << (unsigned)squares[i][0].get() << " " << (unsigned)squares[i][1].get() << " " << (unsigned)squares[i][2].get() << "\t";
            std::cout << (unsigned)squares[i][3].get() << " " << (unsigned)squares[i][4].get() << " " << (unsigned)squares[i][5].get() << "\t";
            std::cout << (unsigned)squares[i][6].get() << " " << (unsigned)squares[i][7].get() << " " << (unsigned)squares[i][8].get() << std::endl;
        }
    ...
    bool attempt_solve() {
        ...
        while (!flag) {
            ...
            if (checkDone()) {
                ...
                auto pv = squares[i][j].get_possible_values();
                auto it = pv.begin();
                std::uniform_int_distribution<std::mt19937::result_type> dist(0, pv.size() - 1);
